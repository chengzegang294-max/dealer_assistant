# pv_corr_state_p0_bar_window_stub_acceptance_v1
...
- 记录 `PV Corr State P0` 的 `append_from_bar_window` 接口空壳已完成只读 dry-run 验证。
- 历史目标不是接 repo-first 历史外部 bar window，而是确认接口壳、参数口径、目标 CSV 指向都成立。
...
python .\12_tooling_runtime_archive\batch_09_legacy_analysis_migration__20260707\lifted_trading_analysis\12_工具运行时_TOOLING_RUNTIME\pv_corr_state_p0_v1\pv_corr_state_p0_append_from_bar_window_stub_v1.py --dry-run
```
...
- dry-run 输出确认：
  ...
  - `source_kind = pending_bar_window_series`
  - `allow_live_binding = False`
  - `allow_bar_window_append = False`
...
- 若继续推进同一条线，最顺动作是：
  - 再决定 bar-window 输入契约是否要显式拆成独立 CSV/JSON schema
