...
- 记录 `PV Corr State P0` 首批 `params template + append stub` 的历史 `dry-run + persist` 验收结论。
...
python .\12_tooling_runtime_archive\batch_09_legacy_analysis_migration__20260707\lifted_trading_analysis\12_工具运行时_TOOLING_RUNTIME\pv_corr_state_p0_v1\pv_corr_state_p0_runtime_append_stub_v1.py
python .\12_tooling_runtime_archive\batch_09_legacy_analysis_migration__20260707\lifted_trading_analysis\12_工具运行时_TOOLING_RUNTIME\pv_corr_state_p0_v1\pv_corr_state_p0_runtime_append_stub_v1.py --persist
```
...
- 已成功读取：
  ...
  - `real_input_samples\pv_corr_state_p0_proof_output_v1.csv`
- 已成功校验：
  - `pv_corr_state_p0_fields_runtime_v1.csv` 表头与 `v1` 合同一致
- 已成功完成 dry-run 内存追加：
...
- 历史上还不能宣称：
  - 已接 repo-first 历史价量序列滚动重建
  - 已成为 repo-first 历史入场字段

## 当时下一步（历史计划备注）

- 若继续推进同一条线，最顺动作是：
  - 再决定是否值得补 `append_from_bar_window` 独立脚本
