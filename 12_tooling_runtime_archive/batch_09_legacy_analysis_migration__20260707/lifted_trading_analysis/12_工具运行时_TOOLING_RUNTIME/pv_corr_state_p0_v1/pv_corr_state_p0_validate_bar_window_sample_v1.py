...
def validate_rows(rows: list[dict[str, str]], expected_header: list[str]) -> dict[str, object]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        pv_corr_score = to_float(row["pv_corr_score"].strip(), "pv_corr_score")
        if not -1.0 <= pv_corr_score <= 1.0:
            raise ValidationError(f"row {index}: pv_corr_score out of range: {pv_corr_score}")

        to_float(row["price_net_change"].strip(), "price_net_change")
        to_float(row["volume_net_change"].strip(), "volume_net_change")

        volume_kind = row["input_volume_kind"].strip()
        if volume_kind not in ALLOWED_VOLUME_KINDS:
            raise ValidationError(f"row {index}: invalid input_volume_kind: {volume_kind}")
        volume_kinds.add(volume_kind)
    ...
    return {
        ...
        "input_volume_kinds": sorted(volume_kinds),
    }
...
def main() -> None:
    ...
    header_path = runtime_dir / params["bar_window_input_header_file"]
    ...
    print(
        "input_volume_kinds={0}".format(
            json.dumps(summary["input_volume_kinds"], ensure_ascii=True)
        )
    )
