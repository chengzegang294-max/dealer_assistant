# pv_corr_state_p0_bar_window_mapping_acceptance_v1
...
- 记录 `PV Corr State P0` 第一版 `bar window sample -> append-ready row` 映射校验已经通过。
...
- 映射输出确认：
  ...
  - `pv_sync_state_counts = {"confirm": 1, "diverge": 1, "neutral": 1}`
...
- `PV Corr State P0` 已具备：
  - 从 `bar window sample input` 到 `append-ready output row` 的只读映射校验闭环
  - 与 `pv_corr_state_p0_fields_output_header_v1.txt` 对齐的字段顺序
- 历史上还不能宣称：
  - 已把映射结果回写仓内运行产物
