# pv_corr_state_p0_replay_preview_acceptance_v1
...
- 记录 `PV Corr State P0` 第一版 replay preview 导出的历史验收结果。
...
- preview 导出输出确认：
  ...
  - `preview_csv = pv_corr_state_p0_replay_preview_rows_v1.csv`
  - `before_row_count = 5`
  - `preview_row_count = 3`
  - `after_replay_row_count = 8`
  - `preview_trade_ids = ["PVC_BAR_SAMPLE_001", "PVC_BAR_SAMPLE_002", "PVC_BAR_SAMPLE_003"]`
  - `runtime_write_attempted = false`
  - `archive_preview_export_passed = true`
- preview csv 当时内容：
  - `PVC_BAR_SAMPLE_001,confirm,up_confirm,none,valid,strong_positive_corr_same_direction`
  - `PVC_BAR_SAMPLE_002,diverge,mixed,price_up_volume_down,valid,strong_corr_opposite_direction`
  - `PVC_BAR_SAMPLE_003,neutral,none,price_down_volume_up,valid,low_absolute_corr`

## 历史可接受结论

- `PV Corr State P0` 在旧链路中已具备：
