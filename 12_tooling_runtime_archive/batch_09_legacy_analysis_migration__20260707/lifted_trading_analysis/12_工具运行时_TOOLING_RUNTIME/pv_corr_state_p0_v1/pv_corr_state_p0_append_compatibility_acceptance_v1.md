# pv_corr_state_p0_append_compatibility_acceptance_v1
...
- 记录 `PV Corr State P0` 第一版 `sample mapping -> append stub` 无写入兼容性联调已经通过。
...
- 已成功校验：
  - `pv_corr_state_p0_fields_runtime_v1.csv` 表头与 append 合同一致
