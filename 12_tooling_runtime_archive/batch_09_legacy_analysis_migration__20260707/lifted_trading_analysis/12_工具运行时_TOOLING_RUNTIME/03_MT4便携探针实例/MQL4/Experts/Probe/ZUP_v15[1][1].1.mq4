...
extern color ExtPesavento=DarkBlue;
extern color ExtGartley866=DarkBlue;
...
void fibo_standart(double fiboPrice,double fiboPrice1)
{
   ...
   ObjectSetFiboDescription(nameObj, 1, "23.6  -->  "+DoubleToStr(fiboPrice*0.236+fiboPrice1, 4) ); 

   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+2,0.382);
   ObjectSetFiboDescription(nameObj, 2, "38.2  -->  "+DoubleToStr(fiboPrice*0.382+fiboPrice1, 4) ); 
         
   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+3,0.5);
   ObjectSetFiboDescription(nameObj, 3, "50.0  -->  "+DoubleToStr(fiboPrice*0.5+fiboPrice1, 4) ); 

   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+4,0.618);
   ObjectSetFiboDescription(nameObj, 4, "61.8  -->  "+DoubleToStr(fiboPrice*0.618+fiboPrice1, 4) ); 

   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+5,0.764);
   ObjectSetFiboDescription(nameObj, 5, "76.4  -->  "+DoubleToStr(fiboPrice*0.764+fiboPrice1, 4) ); 
         
   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+6,1.0);
   ObjectSetFiboDescription(nameObj, 6, "100.0  -->  "+DoubleToStr(fiboPrice+fiboPrice1, 4) ); 
         
   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+7,1.618);
   ObjectSetFiboDescription(nameObj, 7, "161.8  -->  "+DoubleToStr(fiboPrice*1.618+fiboPrice1, 4) ); 
         
   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+8,2.618);
   ObjectSetFiboDescription(nameObj, 8, "261.8  -->  "+DoubleToStr(fiboPrice*2.618+fiboPrice1, 4) ); 

   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+9,4.236);
   ObjectSetFiboDescription(nameObj, 9, "423.6  -->  "+DoubleToStr(fiboPrice*4.236+fiboPrice1, 4) ); 

   ObjectSet(nameObj,OBJPROP_FIRSTLEVEL+10,6.854);
   ObjectSetFiboDescription(nameObj, 10, "685.4  -->  "+DoubleToStr(fiboPrice*6.854+fiboPrice1, 4) ); 
      ...
      if (fs==1 && fsp==2) // Тредн сменился с нисходящего на восходящий
        {
         ...
         zz[bi]=Low[bi];
         zzL[bi]=Low[bi];
         if (i>0) {if (PeakDet) for (n=bip; n>=bi; n--) {lam[n]=Low[bip];}}
         hLast=High[i];
         aip=ai; 
         taip=Time[ai];
         ai=i;
      ...
      if (fs==2 && fsp==1) // Тредн сменился с восходящего на нисходящий
        {
         ...
         zz[ai]=High[ai];
         zzH[ai]=High[ai];
         if (i>0) {if (PeakDet) for (n=aip; n>=ai; n--) {ham[n]=High[aip];}}
         lLast=Low[i];
         bip=bi; 
         tbip=Time[bi];
         bi=i;
