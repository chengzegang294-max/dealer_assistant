...
BEYOND_CARD_PATH = RUNTIME_DIR / "n02_ib_or_beyond_multi_session_persistence_card_v1.md"
BEYOND_CARD_SUMMARY_PATH = RUNTIME_DIR / "n02_ib_or_beyond_multi_session_persistence_card_summary_v1.json"
NOT_BEYOND_CARD_PATH = RUNTIME_DIR / "n02_ib_or_not_beyond_multi_session_stability_card_v1.md"
NOT_BEYOND_CARD_SUMMARY_PATH = RUNTIME_DIR / "n02_ib_or_not_beyond_multi_session_stability_card_summary_v1.json"
...
def summarize_rows(
    ...
    for row in rows:
        ...
        mode_counts[mode] += 1
        if row["second_next_session_first_30m_bar_count"] == "30":
            bar_count_30_rows += 1
        if row["second_next_session_first_bar_expected_side"] == "1":
            first_bar_expected_rows += 1

    return {
        "producer": "n02_ib_or_multi_session_persistence_and_stability_cards_p0_build_v1.py",
        "scope": scope,
        ...
        "second_next_session_first_30m_bar_count_30_rows": bar_count_30_rows,
...
def render_card(
    ...
    lines = [
        ...
        "- `second_next_session_first_30m_bar_count_30_rows`：`{0}`".format(summary["second_next_session_first_30m_bar_count_30_rows"]),
...
def build_multi_session_decision_lines(summary: Dict[str, object], beyond_branch: bool) -> List[str]:
    status_counts = dict(summary["status_counts"])
    rows = int(summary["rows"])
    if beyond_branch:
        persistent_rows = int(status_counts.get("second_next_session_first_30m_all_closes_beyond_prior_ib", 0))
        not_persistent_rows = int(status_counts.get("second_next_session_first_30m_not_all_closes_beyond_prior_ib", 0))
        ...
        return [
            ...
            "当前 `{0}/{1}` 行满足持续外侧，`{2}/{1}` 行不满足，`{3}/{1}` 行缺第二个同类 session 数据。".format(
                persistent_rows,
                rows,
                not_persistent_rows,
                missing_rows,
    ...
    stable_rows = int(status_counts.get("second_next_session_first_30m_all_closes_inside_prior_ib", 0))
    unstable_rows = int(status_counts.get("second_next_session_first_30m_not_all_closes_inside_prior_ib", 0))
    missing_rows = int(status_counts.get("missing_second_next_session_first_30m_data", 0))
    return [
        "`not_beyond multi-session stability` 当前只说明：第二个同类 session 首 30 分钟是否整体仍在前一日 `IB` 内侧或边界。",
        "当前 `{0}/{1}` 行满足稳定内侧，`{2}/{1}` 行不满足，`{3}/{1}` 行缺第二个同类 session 数据。".format(
            stable_rows,
            rows,
            unstable_rows,
            missing_rows,
...
def main() -> None:
    ...
    beyond_summary = summarize_rows(
        rows=beyond_rows,
        scope="REOPEN_B9_N02_IB_OR_BEYOND_MULTI_SESSION_PERSISTENCE_CARD_P0",
    ...
    beyond_card_path.write_text(
        render_card(
            title="n02_ib_or_beyond_multi_session_persistence_card v1",
            purpose_line="把 `beyond multi-session persistence` 观察固定成独立说明卡。",
            summary=beyond_summary,
            decision_lines=build_multi_session_decision_lines(beyond_summary, beyond_branch=True),
        ),
        encoding="utf-8",
    )
    ...
    not_beyond_summary = summarize_rows(
        ...
        scope="REOPEN_B9_N02_IB_OR_NOT_BEYOND_MULTI_SESSION_STABILITY_CARD_P0",
    ...
    not_beyond_card_path.write_text(
        render_card(
            title="n02_ib_or_not_beyond_multi_session_stability_card v1",
            purpose_line="把 `not_beyond multi-session stability` 观察固定成独立说明卡。",
            summary=not_beyond_summary,
            decision_lines=build_multi_session_decision_lines(not_beyond_summary, beyond_branch=False),
        ),
        encoding="utf-8",
    )
