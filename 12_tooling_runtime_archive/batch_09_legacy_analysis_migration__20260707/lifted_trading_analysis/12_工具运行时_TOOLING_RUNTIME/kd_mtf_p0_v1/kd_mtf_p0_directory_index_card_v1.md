...
- ARCHIVE_ONLY_RUNTIME_MIRROR: 本卡只用于说明旧 `kd_mtf_p0_v1` 快照目录的历史结构，不作为当前目录级总入口。
- 当前 repo-first 入口先看：
...
- 这张卡只负责说明 `kd_mtf_p0_v1` 目录里每份文件当时做什么、先读什么、哪些先不要碰。
- 目标是保留旧目录追溯关系，不再把该目录当成当前默认进入 `DY-R1` 的第一跳。
