...
| 字段 | 内容 |
|------|------|
| **书名** | 《市场轮廓理论》（中文译本，对应原著 *Market Profile* 体系） |
| **作者/体系** | J. Peter Steidlmayer / CBOT 市场轮廓理论 |
| **语言** | 中文 |
| **A2 组定位** | 市场结构 / 日内结构 / 拍卖理论基础 |
| **CUTPACK 版本** | v2_r2 |
| **对应原始文件** | CUTPACK__A2__CN__市场轮廓理论__part2__v2_r1.md（历史修订版） |
| **处理人** | Kimi |
| **处理日期** | 2025-06-17 |
| **真值状态** | `partial_anchor_cutpack`。仅 FineReader OCR EPUB 中可复现的章节段落可视为局部原文锚点；其余条目属于知识推断或待人工核对。 |
| **关键说明** | 本文件基于中文译本《市场轮廓理论》的 Part 2 内容整理。当前可用真值源仅为 FineReader OCR EPUB 的局部章节文本；原始扫描 PDF 无文字层，calibre EPUB 为纯图片伪 EPUB。RETAINED_EXCERPTS 中仅 OCR 可复现的条目可视为局部原文锚定，其余必须按“知识推断，无原文真值”口径处理。 |
...
- **A2-1**：市场轮廓理论（Market Profile）是 A2 组“市场结构/日内结构”模块的**基石材料**。其提出的拍卖市场模型、价值区间、控制点、初期平衡、日类型分类等概念，是后续所有日内结构判断（如开盘类型、响应/主动行为、3I 日、剧变、缺口）的底层框架。
- **与英文原著的关系**：A2 组同时包含 Dalton《Mind Over Markets》和《Markets in Profile》以及 Harris《Trading and Exchanges》。中文译本《市场轮廓理论》是 MP 体系在中文语境下的早期传播版本，其概念体系与 Dalton 两书一致，但表述更偏 CBOT 原始术语（如 TPO 字母、风险套利商、叫价序列）。
- **使用边界**：本材料为“概念框架+可量化规则”双重用途，不是交易信号系统。它提供的是日内结构的**描述语言**和**判断条件**，策略需另外叠加（如 A3 组的趋势判断或 B1 组的因子评分）。

## 适用范围与限制

- **适用**：日内/短线交易者对开盘结构、日类型、价值区间、支撑/阻力、缺口类型的判断。
- **不适用**：纯宏观趋势判断（无周/月级别 VA 序列）、纯基本面分析、跨品种套利（原文以单一期货合约为主）。
- **A股适配**：原文基于 CBOT 债券/期货市场（30分钟字母、每半小时叫价、无午休）。A股需做时段映射（30分钟K线对应A-H共8个时段，午休11:30-13:00与期货不同）。

---

# RETAINED_EXCERPTS

以下摘录为 **局部 OCR 锚定 + 局部知识推断** 的混合整理结果，按章节归类，并标注对应章节。凡无法在 FineReader OCR EPUB 中复现的条目，应视为“知识推断，无原文真值”，后续需逐条人工核对。

---

## Excerpt 1：初期平衡（Initial Balance）—— 第一章

> "由A、B分区（第一小时）组成，主要由风险套利商建立。风险套利商试图寻找满足买卖双方的价位。IB阶段成交量可达全日50%。"
>
> —— 第一章，关于初期平衡的定义。

**保留原因**：IB 是全天结构判断的起点。所有后续扩展、日类型分类、开市型态均依赖于 IB 的宽度与位置。

---

## Excerpt 2：价值区间（Value Area）—— 第一章 + 附录

> "每个交易日总成交70%发生的区间。从成交量最大价位开始，向上下两侧累加相邻价位，直到覆盖70%。"
>
> —— 第一章第三节 + 附录"价值区间的计算方法"。

> "步骤：1. 确定成交量最大的价位，取其成交量为基数；2. 分别计算该价位之上(之下)的两个价位发生的成交量之和，并将两和数相比较，较大的一方和数与基数相加，成为新的基数，对应两个价位成为价值区间的一部分；3. 将基数与总成交相比较，若未达到70%，则重复步骤2，直至达到70%为止。"
>
> —— 附录"价值区间的计算方法"（成交量法）。

**保留原因**：价值区间是 MP 的核心概念之一，也是后续所有跨日分析（VA Rule、响应/主动判断）的基准。

---

## Excerpt 3：控制点（POC）—— 第一章

> "最长的且最接近价格幅度中部的TPO线。当日成交最活跃、最合理的价格。"
>
> —— 第一章第三节。

**保留原因**：POC 是日内价值的核心锚点。多日线 POC 的移动方向可作为趋势判断依据。

---

## Excerpt 4：价格幅度扩展（Range Extension）—— 第二章

> "价格运动超出IB阶段价格幅度。表示长线买方/卖方参与，市场发生变化。风险套利商不是运动主因。"
>
> —— 第二章，关于价格幅度扩展的定义。

**保留原因**：扩展是区分“平衡日”与“趋势日”的关键特征。扩展方向也是日内趋势判断的直接依据。

---

## Excerpt 5：单字母尾部（Tails）—— 第二章

> "极端低位出现的单字母分区。长线买方热烈响应低于价值的价位。买方竞争使价格快速上升。尾部至少需两个TPO，且在非收盘时段出现。"
>
> —— 第二章，买入尾部定义。

> "极端高位出现的单字母分区。长线卖方在高价抛售，价格很快降低。"
>
> —— 第二章，卖出尾部定义。

**保留原因**：尾部是“拒绝/接受”信号的最直观形式。尾部越长（更多价位单字母），拒绝信号越强。

---

## Excerpt 6：收市分区（Closing Range）—— 第二章

> "M分区（最后一个时段）反映当日整体市场情绪。与次日开市价位比较，观察情绪变化。"
>
> —— 第二章，收市分区定义。

**保留原因**：次日开盘价 vs 收市分区 = 情绪连续/变化判断。

---

## Excerpt 7：七种日类型—— 第二章

> **平衡交易日（Normal Day）**："初期平衡阶段价格幅度很宽，全天在IB内震荡。通常由开盘消息引起，长线参与者过早介入。"
>
> **变形平衡交易日（Normal Variation）**："IB幅度比平衡日小，之后被长线突破。突破常发生在市场前半段。"
>
> **强趋势交易日（Trend Day）**："长线从开盘到收盘单向控制。每个时段都推向更高/更低（单方市）。TPO轮廓狭长，成交量高。"
>
> **弱趋势交易日 / 双重分布（Double Distribution）**："IB窄，后期长线介入推动价格到新水平，形成第二价值区。两区被单字母TPO分开。"
>
> **无趋势交易日（Nontrend Day）**："IB窄，像趋势日但无扩展。市场等待消息。参与者信心不足，成交低。"
>
> **标准中立交易日（Neutral Day）**："买卖双方都在两端扩展，收盘在中间。双方力量均衡。"
>
> **高/低收中立交易日（Neutral-Extreme Day）**："买卖双方都在两端扩展，但收盘在极端。收市方获胜。"
>
> —— 第二章，七种日类型定义。

**保留原因**：日类型是日内结构判断的顶层分类。每种日类型对应不同的次日预期和交易策略。

---

## Excerpt 8：四种开市型态—— 第三章

> **驱动开市（Open-Drive）**："开盘后价格立即向一个方向有力运动，不反向试探。长线预先决策。"
>
> **试探驱动开市（Open-Test-Drive）**："开市后在已知参考点外探测，确认无新动向后，迅速返回并穿过开市价位，向反方向运动。"
>
> **失败反转开市（Open-Rejection-Reverse）**："开市后沿一个方向运动，遇反向强力量，反转回开盘区间。"
>
> **无方向感开市（Open-Auction）**："开市价在前一交易日价格幅度内（或外），随机叫价，无方向感。通常发展为无趋势/平衡/中立日。"
>
> —— 第三章，四种开市型态定义。

**保留原因**：开市型态是开盘后30分钟内最重要的判断对象。它决定了当日后续发展的初始概率分布。

---

## Excerpt 9：响应与主动行为—— 第三章/第四章

> **响应买方（Responsive Buying）**："价格低于价值区间时，长线买方介入，使价格回升。"
>
> **响应卖方（Responsive Selling）**："价格高于价值区间时，长线卖方介入，使价格回落。"
>
> **主动买方（Initiative Buying）**："在价值区内或价值区外推动价格上升，试图建立新价值。"
>
> **主动卖方（Initiative Selling）**："在价值区内或价值区外推动价格下降，试图建立新价值。"
>
> —— 第三章/第四章，响应与主动行为定义。

**保留原因**：响应=价值回归力量；主动=趋势延续力量。这是 MP 行为分类的核心二分法。

---

## Excerpt 10：31 交易日（3I Day）—— 第八章

> **31买入交易日**："自发买入尾部 + 自发买入TPO + 自发买入价格幅度扩展。三个方向信号一致。"
>
> **31卖出交易日**："自发卖出尾部 + 自发卖出TPO + 自发卖出价格幅度扩展。"
>
> —— 第八章第一节。

> 统计数字："次日94% 90分钟内好于前日VA；97%全日≥前日VA。"
>
> —— 第八章第一节，关于31买入交易日的统计。

**保留原因**：31 日是 MP 中统计胜率最高的特殊结构之一，是“高概率次日持仓”策略的核心触发器。

---

## Excerpt 11：价值区间规则—— 第八章

> "前日价值区顶部提供支持，底部提供阻力。重新进入价值区且被接受（双TPO）→可能突破。开市价越接近价值区，突破可能性越大。区间狭窄更易突破。"
>
> —— 第八章第三节。

**保留原因**：跨日关键位系统。次日开盘价相对于前日VA的位置，是支撑/阻力/突破判断的直接依据。

---

## Excerpt 12：剧变（Spikes）—— 第八章

> "最后几个时段价格迅速偏离已形成的价值区。次日剧变区间内开市=确认；区间外同向=方向延续；反向=拒绝。"
>
> —— 第八章第四节。

> 补充："剧变价位首次试探时有效；反复试探后失效。"
>
> —— 第八章第四节，剧变规则补充。

**保留原因**：剧变是尾盘/次日方向判断的重要信号。

---

## Excerpt 13：平衡区突破—— 第八章

> "多个交易日价值区间重叠形成平衡区。突破后价格运动突然且剧烈。"
>
> —— 第八章，平衡区突破定义。

**保留原因**：平衡区突破是“中线趋势跟踪”的触发条件。

---

## Excerpt 14：空跳缺口—— 第八章

> **突破型（Break-Away Gap）**："趋势早期，新长线参与者引起。缺口成为支持/阻力。"
>
> **加速型（Acceleration Gap）**："趋势中途，再次确认方向。"
>
> **衰竭型（Exhaustion Gap）**："趋势最后阶段，几乎人人成为一方，趋势结束。"
>
> —— 第八章，关于三种缺口类型的定义。

**保留原因**：缺口类型直接影响交易策略（跟随/观望/反向）。

---

## Excerpt 15：TPO 计数法—— 第三章

> "控制点上方TPO总数 vs 下方TPO总数。比例估计价值区内买卖失衡。"
>
> —— 第三章，TPO 计数法定义。

> "TPO图字母映射：每半小时一个字母：A=第一半小时，B=第二半小时..."
>
> —— 第三章，TPO 字母映射规则。

> A股映射："A=9:30-10:00, B=10:00-10:30, C=10:30-11:00, D=11:00-11:30, E=13:00-13:30, F=13:30-14:00, G=14:00-14:30, H=14:30-15:00（A股无I，顺延）。"
>
> —— 根据原文规则做的 A股 适配映射。

**保留原因**：TPO 计数是价值区内买卖失衡的直接估计方法。A股 30分钟K线可替代 TPO 字母计数。

---

## Excerpt 16：无趋势/不确定交易日—— 第二章/第三章

> "无趋势：IB窄，无扩展，成交低。不确定：似乎有机会但无真正参考点。"
>
> —— 第二章/第三章，无趋势/不确定日的定义。

> 补充："不确定日更容易诱多，因为似乎有驱动型态但实际无方向。"
>
> —— 第三章，关于不确定日的补充说明。

**保留原因**：两者都是“观望”信号。不确定日=假的驱动型态，容易诱多。

---

## Excerpt 17：特殊图形—— 第五章

> **空头补仓止跌回稳（"P"形）**："价格狂跌后，空头回补（买回）引起反弹，无新买方。图形呈'P'字。补仓结束后价格返回原轨道。"
>
> **多头斩仓狂跌（"b"形）**："上升趋势后，多头平仓（卖出）引起下跌，无新卖方。图形呈'b'字。"
>
> —— 第五章，关于特殊图形模式的定义。

**保留原因**：图形识别需分钟级数据，可作为模式库在数据充足后启用。

---

## Excerpt 18：平衡板（Ledge）—— 第五章/第六章

> "某价位区反复形成单字母/双字母停留，成为日内支撑/阻力。"
>
> —— 第五章/第六章，平衡板定义。

**保留原因**：平衡板=日内关键位。日内某价位多次出现停留（TPO聚集），之后被接受或拒绝。

---

## Excerpt 19：长线极端价位—— 第七章

> "长期趋势中的极端价位，远离已建立的价值。响应参与者在此介入。"
>
> —— 第七章，长线极端价位/价格偏离的定义。

**保留原因**：长线极端=中长线建仓/减仓点。需要多日/多周线 VA 序列。
...
1. **市场结构（Structure）**：由 TPO 字母构成的价格-时间分布图（Profile）。核心元素：价值区间（VA 70%）、控制点（POC）、价格幅度（Range）、单字母尾部（Tails）。
2. **市场控制力量（Control）**：判断谁在控制市场——风险套利商（Locals/短线）还是长线参与者（其他时间框架交易者）。核心判断依据：日类型、开市型态、扩展方向、响应 vs 主动行为。
3. **特殊交易机会（Opportunity）**：基于特定结构识别的高概率场景：3I 日、中立日、价值区间规则、剧变、平衡区突破、缺口。

## 概念 2：价值区间的计算逻辑（成交量法）

条件：从成交量最大价位开始，向上下两侧累加相邻价位，直到覆盖全日总成交量的70%。
...
quant_status：`proxy_quantizable_now` — 可用分钟K线成交量聚合实现，但属近似代理。

## 概念 3：价值区间的计算逻辑（TPO法）

条件：从TPO数量最多的价位（POC）开始，向上下两侧累加相邻价位的TPO数，直到覆盖全日TPO总数的70%。
...
条件：最长的且最接近价格幅度中部的TPO线。当日成交最活跃、最合理的价格。
...
caveats：proxy/approximation。原文的日类型判断依赖于TPO图的完整形态（如“每个时段推向更高”、“TPO轮廓狭长”），仅用OHLCV和简化阈值是近似代理。准确率有限，需持续校准。本清单为判定逻辑，非综合打分引擎。
...
- **Excerpt 1**（初期平衡）→ 所有日内结构判断的起点
- **Excerpt 2–3**（价值区间/控制点）→ 日内价值锚点
- **Excerpt 4–6**（扩展/尾部/收市分区）→ 日内结构判断的四大参照点
- **Excerpt 7**（七种日类型）→ 日内策略的顶层分类框架
- **Excerpt 8**（四种开市型态）→ 开盘策略的触发条件
- **Excerpt 9**（响应/主动行为）→ 价值回归 vs 趋势延续的判断框架
- **Excerpt 10**（31日统计）→ 高胜率次日持仓策略
- **Excerpt 11**（价值区间规则）→ 跨日支撑阻力突破策略
- **Excerpt 12**（剧变）→ 尾盘/跳空次日方向判断策略
- **Excerpt 13–14**（平衡区突破/缺口）→ 中线趋势跟踪策略
- **Excerpt 17**（P/b形）→ 短线反转/假突破识别
- **Excerpt 18**（平衡板）→ 日内关键位识别
- **Excerpt 19**（长线极端价位）→ 中长线建仓/减仓点

---

*End of Part 2*
*File: CUTPACK__A2__CN__市场轮廓理论__part2__v2_r2.md*
