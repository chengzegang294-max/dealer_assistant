...
- language: 中文
- source_type: 书籍（PDF扫描版 OCR）
- source_quality: 中等。原书为图文混排，OCR 后存在部分图注乱码、K 线数据识别噪声，但正文逻辑和术语基本完整可辨。
...
- notes: |
  ...
  原书大量使用 K 线图案例，案例中的图注乱码不影响核心规则提取。
...
- concept: 调整形态几何识别（三角形/菱形/旗形）
  ...
  notes: 需要专门的几何形态识别模块。与 A3-C2 的威科夫/量价结构可联动开发。
...
- concept: 筹码形态与具体 K 线图的精确对应
  ...
  whether_it_is_still_valuable: 是。可与 A3-C2 的威科夫结构分析、量价分析联动开发。
