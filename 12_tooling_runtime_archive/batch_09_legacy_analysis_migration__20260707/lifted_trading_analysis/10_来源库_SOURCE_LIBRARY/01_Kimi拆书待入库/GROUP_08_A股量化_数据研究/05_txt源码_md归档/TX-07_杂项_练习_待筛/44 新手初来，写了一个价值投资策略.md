...
- origin_path: D:\Stock\trading_assistant\12_tooling_runtime_archive\batch_09_legacy_analysis_migration__20260707\lifted_trading_analysis\10_来源库_SOURCE_LIBRARY\01_Kimi拆书待入库\GROUP_08_A股量化_数据研究__SOURCE_RAW\新的参考书\《Python股票量化交易从入门到实践》完整版\1.量化策略代码(99份)（赠品）\44 新手初来，写了一个价值投资策略.txt
...
def trade(context):
   # 初始化买入列表
    ...
    if g.buy_trade_days%g.buy_refresh_rate == 0:
        ...
        ### _入场函数筛选-开始 ###
        buy_lists = [security for security in buy_lists if situation_filter_xiaoyu_ma(security, 'close', 99)]
