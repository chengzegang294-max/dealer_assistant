# S_BUCKET_F002_PROOF_OF_MAPPING_V1

## 角色

- 本目录只用于 `SBKT_F002` 的 `proof-of-mapping`。
- 当前目标是证明 `return_quantile_input + active_trade_ratio_input`
  如何映射到 `F002` 的 guard 输出合同。
...
- 先看 `S_BUCKET_proof_of_mapping_priority_queue_v1.tsv`，确认 `F002` 是当前 `queue_rank=3`。
- 再看 `S_BUCKET_round3_function_core_function_library_entry_v1.tsv`，确认 `F002` 的默认 guard 角色。
- 再看 `S_BUCKET_round3_function_core_f002_guard_fields_v1.tsv`，确认字段级输入输出与禁令列。
