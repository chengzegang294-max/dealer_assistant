param(
    ...
    [string]$ReportStem = "mt5_bar_export_probe",
    [string]$ArchiveTag = "",
...
$expertTarget = Join-Path $dataRoot "MQL5\Experts\MT5BarExportProbe.ex5"
$runtimeSetFileName = "MT5BarExportProbe.runtime.$($Symbol.ToLowerInvariant()).$($ChartPeriod.ToLowerInvariant()).set"
$runtimeSetPath = Join-Path $dataRoot ("MQL5\Profiles\Tester\" + $runtimeSetFileName)
$runtimeIniPath = Join-Path $batchDir "_tmp_mt5_bar_export_runtime.ini"

$expertSource = Join-Path $archiveDir "MT5BarExportProbe.ex5"
$iniTemplateSource = Join-Path $archiveDir "MT5BarExportProbe.ini"
$archiveRoot = ""
if ($ArchiveTag) {
    $archiveRoot = Join-Path $batchDir ("artifacts\mt5_bar_export\" + $ArchiveTag)
}
$exportTimeframeValue = Get-Mt5EnumTimeframeValue -Label $ChartPeriod

if (-not (Test-Path $terminalExe)) {
    throw "terminal64.exe not found: $terminalExe"
}
if (-not (Test-Path $expertSource)) {
    throw "MT5BarExportProbe.ex5 not found: $expertSource"
}
if (-not (Test-Path $iniTemplateSource)) {
    throw "MT5BarExportProbe.ini not found: $iniTemplateSource"
}
...
Write-Step "symbol=$Symbol chart_period=$ChartPeriod from=$FromDate to=$ToDate"
Write-Step "common files dir: $commonFilesDir"

$baselineTime = Get-Date
$csvBefore = Get-LatestFile -PathPattern (Join-Path $commonFilesDir "MT5_bars_export_*.csv")
...
Write-Step "deploying MT5 bar export expert into data folder"
...
function Write-RuntimeSet {
    $desiredText = (
        @(
            ...
            "InpFilePrefix=MT5_bars_export"
            "InpExportTf=$exportTimeframeValue"
...
function Write-RuntimeIni {
    ...
    $iniText = Remove-IniValue -Text $iniText -Key "InpUseCommonFiles"
    $iniText = Remove-IniValue -Text $iniText -Key "InpFilePrefix"
    $iniText = Remove-IniValue -Text $iniText -Key "InpExportTf"
...
function Invoke-Mt5ConfigRun {
    ...
    Write-Step "writing runtime tester set with InpExportTf=$exportTimeframeValue ($ChartPeriod)"
    Write-RuntimeSet
    ...
    $process = Start-Process -FilePath $terminalExe -ArgumentList @("/config:$runtimeIniPath") -WorkingDirectory $InstallRoot -PassThru
...
try {
    ...
    $newCsv = Wait-ForNewArtifact -Pattern (Join-Path $commonFilesDir "MT5_bars_export_*.csv") -BaselineTime $baselineTime -TimeoutSeconds 20
    ...
    $archiveSummary = @{
        ...
        producer = "MT5BarExportProbe.ex5"
        producer_source = (Join-Path $archiveDir "MT5BarExportProbe.mq5")
    ...
    Write-Host ""
    Write-Host "result=best_effort_mt5_bar_export_run"
    ...
    if (Test-Path $runtimeIniPath) {
        try {
            Remove-Item $runtimeIniPath -Force -ErrorAction Stop
        } catch {
            Write-Step ("runtime ini cleanup skipped: " + $_.Exception.Message)
        }
    }
}
