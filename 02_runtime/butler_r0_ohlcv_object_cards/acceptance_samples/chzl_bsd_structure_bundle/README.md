...
- `auto_series/*.tsv`
  - 由 `build_chzl_structure_series_v1.py` 从真实日线样本自动生成的半自动结构序列
...
- `auto_structure_series` 是默认优先输入，由真实日线样本自动生成。
...
- 先让 `run_chzl_bsd_sample_stub_v1.py` 优先消费 `auto_series`，再用 `seed` 做同日期覆盖。
- 之后再决定是否补 `run_chzl_bsd_on_sample.py` 或并入统一 runner。
