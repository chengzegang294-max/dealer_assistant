...
- `KELLY`：动态仓位与风险预算（依赖交易日志）
- `MFLOW`：资金流向（需要数据可用性审计与降级路径；字段合同已落）
- `CHZL_BSD`：结构信号（分型/笔/中枢；字段合同已落）
- `BPB`：突破回调质量
- `VP`：成交量分布
- `TKR7`：AO 背离
- `YTC`：微观结构（A股需明确数据降级；字段合同已落）
- `INSTB`：机构行为（方法层；字段合同已落）
- `ATRATIO`：A股纯多头语境下默认跳过或降级；字段合同已落
