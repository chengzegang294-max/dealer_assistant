# Batch100 Non-Kimi Boundary (00_外部公开资料与方法论参考)
...
- 本批次按 `non_kimi boundary batch` 口径保留。
- 当前已形成三条稳定提升线：
  - `Batch9=公开指标资料 -> 合同层 / 重开层`
  - `NFTRADEZ=视频方法资料 -> 方法参考 / 词典 / 盘前模板层`
  - `Smile_SMC=分析系统总纲 / 逐帧信息流 / 原始总结稿 -> 方法参考层`
- `Smile_SMC` 的 `video_screenshots/` 当前继续保留 raw snapshot 原位，不单独切第二个证据包，第一轮证据层入口保持 `逐帧运行过程_文字化_v1.md`。
