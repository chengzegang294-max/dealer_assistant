"""
calc_daily_bar_pool_with_indicators_v1.py
作用：输入代码池日线 List[dict] → 输出结构化日线+MA/EMA/MACD/RSI/BOLL
"""
...
def enrich_rows(rows: List[Dict]) -> List[Dict]:
    ...
    order: List[str] = []
    for r in rows:
        code = _pick(r, "code", "stock_code", "stock_code_6d", "代码", "股票代码")
        if not code:
            continue
    ...
    for code in order:
        ...
        if not closes:
            for it in items:
                ...
                row["code"] = code
                row["name"] = _pick(it, "name", "stock_name", "名称", "股票简称")
                ...
                row["volume"] = _pick(it, "volume", "vol", "vol_share", "成交量")
...
def clean_stock_code(v: str) -> str:
    if not v:
        return ""
    v = v.strip()
...
def main(argv=None) -> int:
    ...
    td = args.trade_date
    matches = list(raw_root.glob(f"bar_pool_raw_{td}.csv")) + list(raw_root.glob("bar_pool_raw_*.csv"))
    if not matches:
        print("[1_代码池] 未找到 bar_pool_raw_*.csv，SKIP（后面填CSV再跑）")
        return 0
    src = matches[0]
    print(f"[1_代码池] 读原始CSV：{src.name}")
    raw_rows: List[Dict] = []
    with src.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        for r in reader:
            code = clean_stock_code(_pick(r, "代码", "股票代码", "code", "stock_code", "stock_code_6d"))
            if not code:
                continue
            raw_rows.append({
                "code": code,
                "name": _pick(r, "名称", "股票简称", "name", "stock_name"),
                "trade_date": td,
                "open": _pick(r, "今开", "开盘价", "open"),
                "high": _pick(r, "最高", "最高价", "high"),
                "low": _pick(r, "最低", "最低价", "low"),
                "close": _pick(r, "最新", "现价", "收盘", "收盘价", "close", "昨收"),
                "volume": _pick(r, "成交量", "volume", "vol", "vol_share"),
                "amount": _pick(r, "成交额", "amount", "amt", "amt_yuan"),
            })
    rows_out = enrich_rows(raw_rows)
    out_p = out_root / f"a_share_daily_bar_pool_{td}.csv"
    if args.apply and rows_out:
        ...
        print(f"[1_代码池] WROTE {out_p} → 共 {len(rows_out)} 行")
    else:
        print(f"[1_代码池] DRY_RUN/空：匹配 {len(rows_out)} 行；加--apply写输出")
    return 0
...
def run_acceptance() -> int:
    ...
    for _name, cols in groups:
        ...
        if ok:
            ...
            fail_n += 1
    total = len(groups)
    print(f"【calc_daily_bar_pool_with_indicators_v1 验收】通过 {pass_n}/{total} 行，FAIL {fail_n}/{total} 行，最大绝对误差 = {max_abs}")
